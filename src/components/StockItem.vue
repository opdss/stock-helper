            ...
            <q-btn dense @click.stop round flat icon="more_vert">
                <q-menu>
                    <q-list dense style="min-width: 100px">
                        <q-item clickable v-close-popup @click="$emit(item.isBadge ? 'cancelBadge' : 'setBadge')">
                        ...
                        <q-item clickable v-close-popup tag="a" target="_blank" :href="'https://gu.qq.com/'+item.exCode+'/gp'">
        ...
        <div class="row no-wrap q-pa-md">
            ...
            <div class="column q-ma-xs">
                <span style="font-size:12px" v-for="(p,k) in item.info.buy" v-bind:key="k">
                    买{{k+1}}
                    <font :style="'color:'+(p[0] == item.info.closePrice ? 'black' : (p[0] > item.info.closePrice ? 'red' : 'green'))">{{p[0]}}</font> -
                    <font>{{p[1]}}</font>
                </span>
